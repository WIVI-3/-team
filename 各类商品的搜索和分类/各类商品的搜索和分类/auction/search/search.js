// pages/auction/search/search.js
Page({
  data: {
    searchQuery: '',
    history: [],  // 存储历史记录
  ...
  Search() {
    ...
    // 跳转到搜索结果页面并传递搜索关键字
    wx.navigateTo({
      url: `/pages/auction/search/searchresults/searchresults?query=${encodeURIComponent(searchQuery)}`,
    });
  },
  ...
  onSelectHistory(e) {
    ...
    this.setData({
     ...
     wx.navigateTo({
      url: `/pages/auction/search/searchresults/searchresults?query=${encodeURIComponent(selectedQuery)}`,
    });
  },
