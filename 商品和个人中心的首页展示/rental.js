...
Page({
  ...
  onLoad() {
    // 假设从后端获取的商品数据包含图片名称
    wx.request({
      url: 'https://101972498yahu.vicp.fun/api/rentalgoods/', // 后端接口URL
      method: 'GET',
      success: (res) => {
 ...
 onPullDownRefresh() {
  ...
  goodListClickHandle(e) {
    ...
    wx.navigateTo({
       url: `/pages/rental/goods/rentalgoods?query=${encodeURIComponent(productId)}`,
    });
  },

  onGoToSearchPage() {
    wx.navigateTo({
      url: '/pages/rental/search/search' // 跳转到搜索页面
    });
  },
  ...
  onGoToelectronics() {
    wx.navigateTo({
      url: '/pages/rental/classification/electronics/electronics' 
    });
  },
  onGoToclothing() {
    wx.navigateTo({
      url: '/pages/rental/classification/clothing/clothing' 
    });
  },
  onGoTobooks() {
    wx.navigateTo({
      url: '/pages/rental/classification/books/books' 
    });
  },
  onGoTosports() {
    wx.navigateTo({
      url: '/pages/rental/classification/sports/sports' 
    });
  },
  onGoToothers() {
    wx.navigateTo({
      url: '/pages/rental/classification/others/others' 
    });
  },
